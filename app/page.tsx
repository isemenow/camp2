...
/** Ссылки, которые можно подменить на реальные deep-links */
const LINKS = {
...
/* ===== Вспомогательные компоненты (всё в одном файле, чтобы точно собралось) ===== */

function GlassDecoration({ className }: { className?: string }) {
  return (
    <div
      className={`pointer-events-none -z-10 absolute rounded-full bg-gradient-to-br from-red-500/25 via-gray-200/40 to-white/30 backdrop-blur-xl ${className}`}
      aria-hidden
    />
  )
}

function MiniGlassAccent() {
  return (
    <svg viewBox="0 0 192 64" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
      <defs>
      ...
      <rect x="0.5" y="0.5" width="191" height="63" rx="14" fill="#fff" opacity="0.7" stroke="#e5e7eb" />
      <rect x="10" y="10" width="80" height="44" rx="10" fill="#fff" opacity="0.5" stroke="#e5e7eb" />
      <rect x="100" y="10" width="82" height="44" rx="10" fill="#fff" opacity="0.5" stroke="#e5e7eb" />
      <rect x="10" y="30" width="172" height="6" rx="3" fill="url(#r)" />
    </svg>
  )
}
      ...
      <section id="route" className="py-16 relative">
        <div className="max-w-6xl mx-auto px-5">
          ...
          <div className="mt-6 grid md:grid-cols-3 gap-6">
            {[{ n:'01', t:'Получите лотерейный билет', d:'При входе предъявите рабочий пропуск и возьмите уникальный билет.' },
              { n:'02', t:'Пройдите минимум 4 активности', d:'Соберите штампы: FIFA, Gran Turismo, VR, йога, художественный/парфюмерный/поэтический CONNECT, AI-воркшоп, вокал, кастом-обвесы, роспись шоперов, гонки на робошарах, AR-квест.' },
              { n:'03', t:'Сдайте билет в CONNECTотрон', d:'Опустите билет у Главной сцены и участвуйте в розыгрыше призов.' }
            ].map((s,i)=> (
      ...
      <section id="contacts" className="py-16 border-t border-gray-100 relative">
        <div className="max-w-6xl mx-auto px-5">
          ...
          <div className="mt-6 grid md:grid-cols-3 gap-6 text-sm">
            {[{ name:'Гаврикова Юлия', role:'Руководитель группы развития корпоративной культуры и внутренних коммуникаций', phone:'+7 (929) 973-72-36' },
              { name:'Мунаева Елена', role:'Менеджер по внутренним коммуникациям', phone:'+7 (966) 337-87-93' },
              { name:'Восторгина Александра', role:'Руководитель группы внутренних мероприятий', phone:'+7 (906) 704-95-94' }
            ].map((c,i)=> (
              <Card key={i} className="border-gray-100 bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/60">
                ...
                <CardContent>
                  ...
                  <div className="flex items-center gap-2 mt-2 text-gray-900 font-medium"><Phone className="h-4 w-4" /> {c.phone}</div>
                </CardContent>
              </Card>
            ))}
          </div>
